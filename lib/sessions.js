...
function Sessions(store, opts, storeOpts) {
	...
	if (!this.opts.hasOwnProperty("cookie")) {
		this.opts.cookie = "uid";
	}
...
Sessions.prototype.httpRequest = function (req, res, data, cb) {
	...
	var cookieName = this.opts.cookie;
	...
	if (!cookies.hasOwnProperty(cookieName)) {
		return this.create(data || {}, function (err, session) {
			...
			setHttpCookie(res, cookieName, session.uid(), session.expires(), sessions.opts.path, sessions.opts.domain);

			return cb(null, session, sessions);
		});
	}

	this.get(cookies[cookieName], function (err, session) {
		if (err) {
